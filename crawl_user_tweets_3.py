...
def get_user_tweets(users):
    ...
    users_out_file = open("user_withId_list_6.txt", 'w')
    ...
    for user in users:
        ...
        num_users += 1
         ...
         #   out_file.close()
          #  out_file = codecs.open("crawled_result/tweet_"+theCurrentTime+"_"+(num_users/10000)+".txt", 'w',
           #                        encoding='utf-8')
        ...
        print "Crawled user %s" % (str(user[0]))
        ...
        if content:
            ...
            thefinal = [[tweet["id_str"], tweet["created_at"], tweet["lang"], tweet["text"].encode("utf-8"),
                         tweet["user"]] for tweet in content]
...
def main():
    ...
    while True:
        ...
        if today == theday:
            print "same day"
            break
        users = read_user_ids("user_withId_list_6.txt")
        user_tweets = []
